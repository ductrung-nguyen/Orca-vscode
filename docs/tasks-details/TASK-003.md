...
export class OrcaValidator {
  ...
  private async runTestJob(binaryPath: string): Promise<{ success: boolean; output: string }> {
    ...
    const validationDir = path.join(this.context.globalStorageUri.fsPath, "validation");
...
`;
    ...
    return new Promise((resolve) => {
      ...
      process.on("close", async (code) => {
        ...
        const success = outputContent.includes("HURRAY") && outputContent.includes("FINAL SINGLE POINT ENERGY");
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (result.success) {
    vscode.window.showInformationMessage(`ORCA ${result.installationDetails?.version} validated successfully!`);
  }
}
```
